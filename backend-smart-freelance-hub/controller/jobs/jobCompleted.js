...
exports.assignJob = async (req, res) => {
  ...
  try {
    ...
    if (user) {
      ...
      res.status(200).json({ message: "Job status updated to completed" });
    } else {
      res.status(400).json({ message: "Client not found" });
    }
  } catch (err) {
    console.error("Error during job completion:", err);
    res.status(500).json({ message: "Failed to set completed job" });
  }
};
