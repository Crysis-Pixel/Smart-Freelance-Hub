...
const ChatComponent = () => {
  ...
  useEffect(() => {
    ...
    const fetchUserList = async () => {
      //gets all users store in database
      try {
  ...
  useEffect(() => {
    socket.on("receiveMessage", (newMessage) => {
      //this function checks whether the message recieved is for them and sets it as read if the message is for them.
      ...
      if (newMessage.senderId === recipientIdRef.current) {
        ...
        if (newMessage.recipientId === senderId && !newMessage.isRead) {
          //checks if message has been set to read, if not then sets it as read.
          console.log("Found a message that is not Read.");
          console.log(
            "newMessage object from frontend:" + JSON.stringify(newMessage)
          );
  ...
  const handleTyping = async (e) => {
    //this function deals with everything regarding notifying users whether the other user is typing
    ...
    if (recipientId) {
      if (!isTyping) {
        //if isTyping is false but user is typing, it is set to true
        setIsTyping(true);
        socket.emit("typing", { userId, recipientId, isTyping: true });
      }
      if (e.target.value === "") {
        //if text box is empty this means user is not typing
        setIsTyping(false);
        socket.emit("typing", { userId, recipientId, isTyping: false });
      }
      setTimeout(() => {
        //a timeout delay function that makes the 'user is typing...' text disappear after 4 seconds
        if (isTyping) {
          setIsTyping(false);
          socket.emit("typing", { userId, recipientId, isTyping: false });
        }
  ...
  const markMessageAsRead = async (messageId, senderId) => {
    //the function that sets a message as read if the other user has read it
    await sleep(1000); //a delay to prevent race conditions resulting in looping.
    console.log(
      `Socket function to update message as read has been called. messageId: ${messageId}`
    );
  ...
  const sendMessage = async () => {
    ...
    if ((message.trim() !== "" || selectedFile) && recipientId) {
      ...
      const response = await fetch("http://localhost:3000/messages/create", {
        //new message is updated in database
        method: "POST",
        body: formData,
      });
  ...
  useEffect(() => {
    const handleTyping = ({ userId, isTyping }) => {
      //a simple function that makes sure that user1 does not see user2 typing when user1 is in a chat with user2.
  ...
  useEffect(() => {
    chatMessages.forEach((msg) => {
      //this is called to set all messages as read when user has opened a chat
      if (msg.receiverId === senderId && !msg.isRead) {
        console.log(
          "User has opened chat, all messages have been set to read."
        );
          ...
          {Array.isArray(chatMessages) &&
            chatMessages.map((msg, index) => (
              ...
              >
                ...
                {msg.messageText}
                <span>
                  {msg.isRead && msg.senderId === senderId ? "✔" : ""}
                </span>{" "}
                {/* Adds a checkmark if message is read */}
                <span>
                  {" " +
                    new Date(msg.timestamp).toLocaleString("en-US", {
                      //this function converts data to a readable format
                      timeZone: "Asia/Dhaka",
                      day: "2-digit",
                      month: "2-digit",
                      year: "numeric",
